...
set.seed(123)
GD_rec <- 
  recipe(GDDiag ~ ., data = train_data_GD)%>% step_smote(GDDiag, over_ratio = 0.25)%>%step_dummy(all_nominal_predictors())%>%step_zv(all_predictors())%>%step_nzv(all_predictors())%>% step_center(all_numeric_predictors())%>%
